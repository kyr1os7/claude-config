...
7c. **Format 内容 (mulai 2026-07-13)** — awali dengan judul dalam kurung 【】 lalu deskripsi: **`【<subject>】<deskripsi>`**. Subject = 件名 ticket Zendesk (untuk CEK) atau ringkasan topik (untuk MANUAL). Contoh: `【新在留カード受け取り】書類を送付いただき確認済み。申請受付番号について案内済み。` Berlaku di `notes` (JSON) dan `tableStorageDaily_内容`.
